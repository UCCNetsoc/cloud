<template>
  <v-container fluid>
        ...
        >
          ...
          <template v-slot:item="row">
            ...
            >
                ...
                <v-container class="pa-0 ma-0">
                  <v-row justify="start" class="ma-0 pa-0 mt-2" no-gutters>
                    <v-col sm="12" md="2" align="center" justify="center">
                      <v-avatar
                        ...
                        class="my-2"
                      >
                        <v-icon v-if="row.item[1].metadata.template_metadata.logo_url===''">
                          mdi-help
                        </v-icon>
                        <img
                          v-else
                          :src="row.item[1].metadata.template_metadata.logo_url"
                        >
                      </v-avatar>
                    </v-col>
                    <v-col sm="12" md="10">
                      <h2 class="pa-0 ma-0">{{row.item[1].hostname}}</h2>
                      <v-row class="mb-3" no-gutters justify="start" align="center">
                        <v-icon size="24" :class="row.item[1].active ? 'green--text mr-1' : 'red--text mr-1'">
                  ...
                  <v-row no-gutters class="ml-1 mb-2">
                    <v-col sm="12" md="12" lg="4">
                      <h4>{{ row.item[1].metadata.template_metadata.title }}</h4>
                    ...
                    <v-col sm="12" md="12" lg="4" align="left">
                      <v-btn
                        x-small
                        class="ma-1 blue"
                        @click="msg = marked(row.item[1].metadata.template_metadata.description)"
              ...
              <td>
                <v-container class="ma-0 pa-0 my-4">
                  <v-row class="ma-0 pa-0" no-gutters>
                    <v-col sm="12" md="12" lg="12">
                      Internal IP: <code>{{row.item[1].metadata.network.nic_allocation.addresses[0].split('/')[0] }}</code><br/>
                      Forwarded ports available on: <code>{{row.item[1].fqdn}}</code>
                    </v-col>
                  </v-row>
                  <v-row no-gutters>
                    <v-col sm="12" md="12" lg="7" no-gutters class="my-2">
                      <v-container v-for="(opts, vhost) in row.item[1].metadata.network.vhosts" :key="vhost" class="ma-0 pa-0">
                        <v-row no-gutters justify="start" align="center">
                          <v-col sm="12" md="12">
                            <a v-if="row.item[1].status == Status.Running" class="white--text" target="_blank" :href="'https://'+vhost">https://{{vhost}}</a>
                            <span v-else class="white--text" target="_blank" :href="'https://'+vhost">https://{{vhost}}</span>
                          </v-col>
                        </v-row>
                        <v-row no-gutters justify="start" align="center">
                          <v-col sm="12" md="10">
                            <span style="font-size: 12px">
                              ({{opts.port}}, {{opts.https === true ? "HTTPS" : "HTTP"}})
                            </span>
                          </v-col>
                          <v-col sm="12" md="2">
                            <v-btn
                    ...
                    <v-col sm="12" md="12" lg="5" no-gutters class="my-2">
                      <v-container class="ma-0 pa-0">
                        <v-container class="ma-0 pa-0" v-for="(internal, external) in row.item[1].metadata.network.ports" :key="external" no-gutters>
                          <v-row no-gutters justify="start" align="center">
                            <v-col sm="12" md="10">
                              <code>:{{external}}</code>
                  ...
                  <v-row no-gutters class="my-2">
                    <v-col no-gutters>
                      <v-avatar
                        ...
                        tile
                      >
                        <v-icon size="48" :class="row.item[1].status == Status.Running ? 'green--text' : 'red--text'">
                          {{row.item[1].status == Status.Running ? 'mdi-play' : 'mdi-stop'}}
                        </v-icon>
                      </v-avatar>
                      <span style="font-size: 16px" :class="row.item[1].status == Status.Running ? 'green--text' : 'red--text'">
    ...
    >
            ...
            >
              <v-list-item-group @change="idx => setConfirmCancelTemplate(idx)">
                <template v-for="(item) in templates">
                  ...
                  >
                    <v-list-item-avatar>
                      <v-img :src="item.metadata.logo_url"></v-img>
                    </v-list-item-avatar>

                    <v-list-item-content>
                      <v-list-item-title v-html="item.metadata.title"></v-list-item-title>
                      <v-list-item-subtitle v-html="'<span class=\'text--primary\'>' + item.metadata.subtitle + '</span><br/>' + getSpecString(item.specs)"></v-list-item-subtitle>
                    </v-list-item-content>
                  </v-list-item>
                </template>
              </v-list-item-group>
                ...
                <v-card-text>
                  <v-row no-gutters justify="start" align="center">
                    <v-col>
                      ...
                      >
                        <img
                          :src="confirmCancel.action.template.metadata.logo_url"
                        >
                      </v-avatar>
                    </v-col>
                    <v-col sm="10">
                      <h3 class="white--text">
                        {{ confirmCancel.action.template.metadata.title + ' ' + typeName }}
                      </h3>
                      <span class="grey--text text--lighten-2">
                        {{ confirmCancel.action.template.subtitle }}<br/>
                      </span>
                      <p v-html="marked(confirmCancel.action.template.metadata.description)"></p>
                      <h4 class="white--text">
      ...
      >
        <p>
          A virtual host forwards web (HTTP &amp; HTTPS) traffic into your instance when a match against a HTTP Host header is made

          This allows you to serve web content to the web for any number of domains you specify
        </p>
        <h3>
        ...
        <p>
          ...
          <br/><br/>
          Virtual Hosts may take a few minutes to apply
        </p>
        <v-text-field
          label='Virtual Host'
...
export default Vue.extend({
  ...
  computed: {
    ...
    whyInstanceRules (): ((v: string) => (string | boolean))[] {
      return [
        (v: string) => !!v || 'Reason required',
        openApiPropertyValidator(openApiGetSchemaProperty('InstanceRequestDetail', 'reason'))
      ]
    },

    vhostRules (): ((v: string) => (string | boolean))[] {
      return [
        (v: string) => !!v || 'Virtual host required',
        (v: string) => VHostValidation.test(v) || 'Must be a valid domain name'
      ]
    },

    hostnameRules (): ((v: string) => (string | boolean))[] {
      return [
        (v: string) => !!v || 'Hostname required',
        openApiPropertyValidator(openApiGetSchemaProperty('InstanceRequest', 'hostname'))
      ]
    },
